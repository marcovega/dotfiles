...
load_ssh_connections() {
    local config_file="$HOME/.config/env-files/.ssh-connections"
    local current_alias=""
    ...
    if [[ ! -f "$config_file" ]]; then
        ...
        echo "Generate SSH connections config using: ./scripts/system/generate-configs.sh"
        return 1
    fi
...
ssh-reload() {
    load_ssh_connections
    echo "SSH connections reloaded from ~/.config/env-files/.ssh-connections"
